...
(** A operação de reverso é distributiva em relação a concatenação, prove 
    esse teorema *)
Theorem concat_rev: forall X (l : list X),
  rev(l) ++ [] = rev(l).
  Admitted.

Theorem concat_assoc: forall X (l1 l2 l3: list X),
  (l1 ++ l2) ++ l3 = l1 ++ (l2 ++ l3).
intros. induction l1. reflexivity.
  simpl. rewrite IHl1. reflexivity.
Qed.
...
Proof.
  intros. induction l1. simpl. rewrite concat_rev. reflexivity.
...
Proof.
  intros. induction l1 as [| l1]. reflexivity.
  simpl. rewrite IHl1. reflexivity.
Qed.
